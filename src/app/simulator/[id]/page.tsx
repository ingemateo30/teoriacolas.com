...
const runSimulationStep = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Simulador de Modelo {currentModel.name}</h1>
        <p className="text-gray-600 mb-4">{currentModel.description}</p>
        
        <div className="flex flex-wrap gap-4">
          <Link href="/simulator/mm1" className={`py-2 px-4 rounded-lg ${modelId === 'mm1' ? 'bg-purple-600 text-white' : 'bg-gray-200'}`}>
            Modelo M/M/1
          </Link>
          <Link href="/simulator/mm2" className={`py-2 px-4 rounded-lg ${modelId === 'mm2' ? 'bg-purple-600 text-white' : 'bg-gray-200'}`}>
            Modelo M/M/2
          </Link>
          <Link href="/simulator/mmc" className={`py-2 px-4 rounded-lg ${modelId === 'mmc' ? 'bg-purple-600 text-white' : 'bg-gray-200'}`}>
            Modelo M/M/c
          </Link>
          <Link href="/simulator/mg1" className={`py-2 px-4 rounded-lg ${modelId === 'mg1' ? 'bg-purple-600 text-white' : 'bg-gray-200'}`}>
            Modelo M/G/1
          </Link>
          <Link href="/simulator/md1" className={`py-2 px-4 rounded-lg ${modelId === 'md1' ? 'bg-purple-600 text-white' : 'bg-gray-200'}`}>
            Modelo M/D/1
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="bg-white shadow-lg rounded-lg p-4">
            <h2 className="text-xl font-semibold mb-4">Visualización de la simulación</h2>
        ...
        <div>
          <div className="bg-white shadow-lg rounded-lg p-4 mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-gray-600 mb-1">Tasa de llegadas (λ):</p>
                <p className="font-semibold">{simulationParams.arrivalRate} clientes/ut</p>
              </div>
              <div>
                <p className="text-gray-600 mb-1">Tasa de servicio (μ):</p>
                <p className="font-semibold">{simulationParams.serviceRate} clientes/ut</p>
              </div>
              <div>
                <p className="text-gray-600 mb-1">Número de servidores:</p>
                <p className="font-semibold">{simulationParams.numServers}</p>
              </div>
              <div>
                <p className="text-gray-600 mb-1">Factor de utilización (ρ):</p>
                <p className="font-semibold">{formatUtilization()}</p>
              </div>
            </div>
          </div>
          
          <div className="bg-white shadow-lg rounded-lg p-4">
            <h2 className="text-xl font-semibold mb-4">Demostraciones disponibles</h2>
            
            <div className="space-y-3">
              {demoScenarios.map((scenario, index) => (
                <div 
                  key={index}
                  className={`p-3 rounded-lg border-2 cursor-pointer hover:bg-gray-50 ${currentScenarioIndex === index ? 'border-purple-500' : 'border-gray-200'}`}
